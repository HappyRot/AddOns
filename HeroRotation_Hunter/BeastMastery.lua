...
local a8 = 0;
local a9 = 0;
local aa = {{O.Intimidation, "Cast Intimidation (Interrupt)", function()
    return true
end}}
...
local function ae(af)
    local ae = 0;
    if af ~= nil then
        for ag in pairs(af) do
            local ah = af[ag]
            if ah:IsInRange(40) and (ah:AffectingCombat() or ah:IsDummy() or ah:NPCID() == 153285) and
                ah:DebuffUp(j(257284)) then
                ae = ae + 1
            end
        end
    end
...
local function ai(aj)
    return aj:DebuffRemains(O.BarbedShotDebuff)
end
local function ak(aj)
    return aj:DebuffStack(O.LatentPoisonDebuff)
end
local function al(aj)
    return aj:DebuffRemains(O.SerpentStingDebuff)
end
local function am(aj)
    if aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
            O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() < Z and
            (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == f("mouseover"):GUID() then
        a9 = 1217200;
        return true
    elseif aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
            O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() < Z and
            (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and N.BeastMastery.TargetSwap == "AutoSwap" and
        aj:GUID() ~= h:GUID() and not v then
        a9 = 999;
        return true
    elseif aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
            O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() < Z and
            (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == h:GUID() then
        a8 = 217200;
        return true
    elseif aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
            O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() < Z and
            (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) then
        return true
    end
end
local function an(aj)
    if (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
        O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() < Z and
        (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == f("mouseover"):GUID() then
        a9 = 1217200;
        return true
    elseif (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
        O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() < Z and
        (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and N.BeastMastery.TargetSwap == "AutoSwap" and
        aj:GUID() ~= h:GUID() and not v then
...
local function ao(aj)
    if aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or a0 < 9 or O.WildCall:IsAvailable() and
            O.BarbedShot:ChargesFractional() > 1.8) and (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and
        aj:GUID() == f("mouseover"):GUID() then
        a9 = 1217200;
        return true
    elseif aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or a0 < 9 or O.WildCall:IsAvailable() and
            O.BarbedShot:ChargesFractional() > 1.8) and (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and
        N.BeastMastery.TargetSwap == "AutoSwap" and aj:GUID() ~= h:GUID() and not v then
        a9 = 999;
        return true
    elseif aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or a0 < 9 or O.WildCall:IsAvailable() and
    ...
    elseif aj:DebuffStack(O.LatentPoisonDebuff) > 9 and
        (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or a0 < 9 or O.WildCall:IsAvailable() and
            O.BarbedShot:ChargesFractional() > 1.8) and (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) then
        return true
    end
end
local function ap(aj)
    if (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or a0 < 9 or O.WildCall:IsAvailable() and
        O.BarbedShot:ChargesFractional() > 1.8) and (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and
        aj:GUID() == f("mouseover"):GUID() then
        a9 = 1217200;
        return true
    elseif (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or a0 < 9 or O.WildCall:IsAvailable() and
        O.BarbedShot:ChargesFractional() > 1.8) and (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and
        N.BeastMastery.TargetSwap == "AutoSwap" and aj:GUID() ~= h:GUID() and not v then
        a9 = 999;
        return true
...
local function aq(aj)
    if aj:DebuffRefreshable(O.SerpentStingDebuff) and aj:TimeToDie() > O.SerpentStingDebuff:BaseDuration() and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == f("mouseover"):GUID() then
        a9 = 1271788;
        return true
    elseif aj:DebuffRefreshable(O.SerpentStingDebuff) and aj:TimeToDie() > O.SerpentStingDebuff:BaseDuration() and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and N.BeastMastery.TargetSwap == "AutoSwap" and
        aj:GUID() ~= h:GUID() and not v then
        a9 = 999;
        return true
...
local function ar(aj)
    if (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
        i:BuffStack(O.FrenzyPetBuff) < 3 and (O.BestialWrath:CooldownRemains() > 0 or not o())) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == f("mouseover"):GUID() then
        a9 = 1217200;
        return true
    elseif (i:BuffUp(O.FrenzyPetBuff) and i:BuffRemains(O.FrenzyPetBuff) <= Z + 0.25 or O.ScentofBlood:IsAvailable() and
...
local function as(aj)
    if (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or O.WildCall:IsAvailable() and
        O.BarbedShot:ChargesFractional() > 1.8 or O.BarbedShot:FullRechargeTime() < Z and
        (O.BestialWrath:CooldownRemains() > 0 or not o()) or O.ScentofBlood:IsAvailable() and
        (O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() + Z < 8 and
            O.BestialWrath:CooldownRemains() < 24 + 8 - Z + O.BarbedShot:FullRechargeTime() and o()) or a0 < 9) and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == f("mouseover"):GUID() then
        a9 = 1217200;
        return true
    elseif (O.WildInstincts:IsAvailable() and g:BuffUp(O.CalloftheWildBuff) or O.WildCall:IsAvailable() and
        O.BarbedShot:ChargesFractional() > 1.8 or O.BarbedShot:FullRechargeTime() < Z and
        (O.BestialWrath:CooldownRemains() > 0 or not o()) or O.ScentofBlood:IsAvailable() and
        (O.BestialWrath:CooldownRemains() < 12 + Z and o() or O.BarbedShot:FullRechargeTime() + Z < 8 and
            O.BestialWrath:CooldownRemains() < 24 + 8 - Z + O.BarbedShot:FullRechargeTime() and o()) or a0 < 9) and
...
local function at(aj)
    if aj:DebuffRefreshable(O.SerpentStingDebuff) and h:TimeToDie() > O.SerpentStingDebuff:BaseDuration() and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and aj:GUID() == f("mouseover"):GUID() then
        a9 = 1271788;
        return true
    elseif aj:DebuffRefreshable(O.SerpentStingDebuff) and h:TimeToDie() > O.SerpentStingDebuff:BaseDuration() and
        (aj:AffectingCombat() or aj:IsDummy() or aj:NPCID() == 153285) and N.BeastMastery.TargetSwap == "AutoSwap" and
        aj:GUID() ~= h:GUID() and not v then
        a9 = 999;
        return true
...
local function au()
    if L.TargetIsValid() and a4 then
        ...
        if O.KillCommand:IsCastable() and UnitExists("pet") and a6 then
...
local function av()
    if O.Berserking:IsCastable() and o() and
        (not O.BestialWrath:IsAvailable() or g:BuffUp(O.BestialWrathBuff) or a0 < 16) then
        if p(O.Berserking, nil) then
            a8 = 26297;
            return "Berserking"
        end
    end
    if O.BloodFury:IsCastable() and o() and (g:BuffUp(O.CalloftheWildBuff) or not O.CalloftheWild:IsAvailable() and
        (g:BuffUp(O.BestialWrathBuff) and g:BloodlustUp() or h:HealthPercentage() < 20) or a0 < 16) then
        if p(O.BloodFury, nil) then
            a8 = 20572;
            return "Blood Fury"
        end
    end
    if O.AncestralCall:IsCastable() and o() and (g:BuffUp(O.CalloftheWildBuff) or not O.CalloftheWild:IsAvailable() and
        (g:BuffUp(O.BestialWrathBuff) and g:BloodlustUp() or h:HealthPercentage() < 20) or a0 < 16) then
    ...
    if O.Fireblood:IsCastable() and o() and (g:BuffUp(O.CalloftheWildBuff) or not O.CalloftheWild:IsAvailable() and
        (g:BuffUp(O.BestialWrathBuff) and (g:BloodlustUp() or h:HealthPercentage() < 20)) or a0 < 10) then
        if p(O.Fireblood, nil) then
            a8 = 265221;
            return "Fireblood"
        end
    end
    if N.Commons.Enabled.Potions and o() and w and
        ((g:BuffUp(O.CalloftheWildBuff) or not O.CalloftheWild:IsAvailable() and
        ...
        local aw = L.PotionSelected()
        if aw and aw:IsReady() then
            if p(aw, nil, nil) then
                a8 = 50;
...
local function ax()
    local ay = g:GetUseableTrinkets(R, 13)
    if ay and (g:BuffUp(O.CalloftheWildBuff) or not O.CalloftheWild:IsAvailable() and
        (g:BuffUp(O.BestialWrathBuff) and (g:BloodlustUp() or h:HealthPercentage() < 20)) or a0 < 31) then
        if ay:ID() == X and N.Commons.Enabled.TopTrinket then
    ...
    local az = g:GetUseableTrinkets(R, 14)
    if az and (g:BuffUp(O.CalloftheWildBuff) or not O.CalloftheWild:IsAvailable() and
        (g:BuffUp(O.BestialWrathBuff) and (g:BloodlustUp() or h:HealthPercentage() < 20)) or a0 < 31) then
        if az:ID() == X and N.Commons.Enabled.BottomTrinket then
...
local function aA()
    if O.BarbedShot:IsCastable() then
        if L.CastTargetIf(O.BarbedShot, a1, "max", ak, am, not h:IsSpellInRange(O.BarbedShot)) then
            return "barbed_shot cleave 2"
        end
        if ak(h) and am(h) then
            if p(O.BarbedShot, nil, nil, not a4) then
                a8 = 217200;
                return "barbed_shot cleave 2"
            end
        end
    ...
    if O.BarbedShot:IsCastable() then
        if L.CastTargetIf(O.BarbedShot, a1, "min", ai, an, not h:IsSpellInRange(O.BarbedShot)) then
            return "barbed_shot cleave 4"
        end
        if ai(h) and an(h) then
            if p(O.BarbedShot, nil, nil, not a4) then
                a8 = 217200;
                return "barbed_shot cleave 4"
            end
    ...
    if O.MultiShot:IsReady() and Z - i:BuffRemains(O.BeastCleavePetBuff) > 0.25 then
        if p(O.MultiShot, nil, nil, not a4) then
            a8 = 2643;
            return "Multi-Shot (Cleave - 1)"
        end
    end
    if O.KillCommand:IsReady() and
        (O.KillCommand:FullRechargeTime() < Z and O.AlphaPredator:IsAvailable() and O.KillCleave:IsAvailable()) then
        if p(O.KillCommand) then
            a8 = 34026;
    ...
    if O.Stampede:IsCastable() and H and (g:BuffUp(O.BestialWrathBuff) or a0 < 15) then
        if p(O.Stampede, nil, nil, not a5) then
            a8 = 201430;
    ...
    if O.BestialWrath:IsCastable() and UnitExists("pet") and I then
    ...
    if O.AMurderofCrows:IsReady() then
        if p(O.AMurderofCrows, nil, nil, not a4) then
            a8 = 131894;
            return "A Murder of Crows (Cleave)"
        end
    end
    if O.BarbedShot:IsCastable() then
        if L.CastTargetIf(O.BarbedShot, a1, "max", ak, ao, not h:IsSpellInRange(O.BarbedShot)) then
            return "barbed_shot cleave 26"
        end
        if ak(h) and ao(h) then
            if p(O.BarbedShot, nil, nil, not a4) then
                a8 = 217200;
                return "barbed_shot cleave 26"
            end
    ...
    if O.BarbedShot:IsCastable() then
        if L.CastTargetIf(O.BarbedShot, a1, "min", ai, ap, not h:IsSpellInRange(O.BarbedShot)) then
            return "barbed_shot cleave 28"
        end
        if ai(h) and ap(h) then
            if p(O.BarbedShot, nil, nil, not a4) then
                a8 = 217200;
                return "barbed_shot cleave 28"
            end
    ...
    if O.KillCommand:IsReady() and UnitExists("pet") then
        if p(O.KillCommand) then
            a8 = 34026;
            return "Kill Command (Cleave)"
        end
    ...
    if O.DireBeast:IsReady() then
        if p(O.DireBeast, nil, nil, not a4) then
            a8 = 120679;
            return "Dire Beast (Cleave)"
        end
    end
    if O.SerpentSting:IsReady() then
        if L.CastTargetIf(O.SerpentSting, a1, "min", al, aq, not h:IsSpellInRange(O.SerpentSting)) then
            return "serpent_sting cleave 34"
        end
    end
    if O.Barrage:IsReady() and i:BuffRemains(O.BeastCleavePetBuff) > O.Barrage:ExecuteTime() then
        if p(O.Barrage, nil, nil, not a4) then
            a8 = 120360;
            return "Barrage (Cleave)"
        end
    end
    if O.KillShot:IsReady() then
        if p(O.KillShot, nil, nil, not a4) then
            a8 = 53351;
    ...
    if O.CobraShot:IsReady() and
        (g:FocusTimeToMax() < Z * 2 or g:BuffUp(O.AspectoftheWildBuff) and g:FocusTimeToMax() < Z * 4) then
        if p(O.CobraShot) then
            a8 = 193455;
    ...
    if O.BagofTricks:IsCastable() and o() and (g:BuffDown(O.BestialWrathBuff) or a0 < 5) then
        if p(O.BagofTricks, nil, nil, not a4) then
            a8 = 312411;
...
local function aB()
    if O.BarbedShot:IsCastable() then
        if L.CastTargetIf(O.BarbedShot, a1, "min", ai, ar, not h:IsSpellInRange(O.BarbedShot)) then
            return "barbed_shot st 2"
        end
        if ai(h) and ar(h) then
            if p(O.BarbedShot, nil, nil, not a4) then
                a8 = 217200;
                return "barbed_shot st 2"
            end
    ...
    if O.KillCommand:IsReady() and UnitExists("pet") and a6 and
        (O.KillCommand:FullRechargeTime() < Z and O.AlphaPredator:IsAvailable()) then
        if p(O.KillCommand) then
            a8 = 34026;
    ...
    if O.KillCommand:IsReady() and UnitExists("pet") and a6 then
        if p(O.KillCommand) then
            a8 = 34026;
            return "Kill Command (ST)"
        end
    ...
    if O.BarbedShot:IsCastable() then
        if L.CastTargetIf(O.BarbedShot, a1, "min", ai, as, not h:IsSpellInRange(O.BarbedShot)) then
            return "barbed_shot st 24"
        end
        if ai(h) and as(h) then
            if p(O.BarbedShot, nil, nil, not a4) then
                a8 = 217200;
                return "barbed_shot st 24"
            end
    ...
    if O.SerpentSting:IsReady() then
        if L.CastTargetIf(O.SerpentSting, a1, "min", al, at, not h:IsSpellInRange(O.SerpentSting)) then
    ...
    if O.CobraShot:IsReady() then
        if p(O.CobraShot, nil, nil, not a4) then
            a8 = 193455;
...
local function aC()
    ...
    if N.BeastMastery.AspectoftheWild == "CDs" and o() or N.BeastMastery.AspectoftheWild == "Small CDs" and (o() or t) or
        N.BeastMastery.AspectoftheWild == "Always" then
        G = true
    end
    if N.BeastMastery.Stampede == "CDs" and o() or N.BeastMastery.Stampede == "Small CDs" and (o() or t) or
        N.BeastMastery.Stampede == "Always" then
        H = true
    end
    if N.BeastMastery.BestialWrathCD == "CDs" and o() or N.BeastMastery.BestialWrathCD == "Small CDs" and (o() or t) or
        N.BeastMastery.BestialWrathCD == "Always" then
        I = true
    end
    if N.BeastMastery.Bloodshed == "CDs" and o() or N.BeastMastery.Bloodshed == "Small CDs" and (o() or t) or
        N.BeastMastery.Bloodshed == "Always" then
        J = true
    end
...
local function aD()
    ...
    local X, aE = GetInventoryItemID("player", 13)
    ...
    if L.TargetIsValid() or g:AffectingCombat() then
        ...
        a0 = _;
        if a0 == 11111 then
            a0 = d.FightRemains(Enemies8y, false)
        end
    end
    if a9 > 0 then
        a9 = 0
    ...
    local aF = O.BloodBolt:IsPetKnown() and l.FindBySpellID(O.BloodBolt:ID()) and O.BloodBolt or O.Bite:IsPetKnown() and
                   l.FindBySpellID(O.Bite:ID()) and O.Bite or O.Claw:IsPetKnown() and l.FindBySpellID(O.Claw:ID()) and
                   O.Claw or O.Smack:IsPetKnown() and l.FindBySpellID(O.Smack:ID()) and O.Smack or nil;
    ...
    if g:HealthPercentage() < N.Commons.HealthstoneHP and Q.Healthstone:IsReady() and Q.Healthstone:CooldownRemains() <=
        0 then
    ...
    if g:HealthPercentage() < N.Commons.HealPotHP and Q.CosmicHealPot:IsReady() and Q.CosmicHealPot:CooldownRemains() <=
        0 then
    ...
    if UnitExists("mouseover") and string.find(UnitGUID("mouseover"), 120651) then
        if O.CobraShot:IsCastable() and a4 then
    ...
    K = {333875, 344739, 228318, 332158, 321764, 324914, 326046, 324776, 340544, 324736, 328015, 322433, 334324, 317936,
    ...
    if UnitExists("target") and O.TranqualizingShot:IsCastable() and not D then
        if UnitCanAttack("player", "target") and UnitAffectingCombat("target") and UnitIsDead("target") ~= true and
            O.TranqualizingShot:IsCastable() and O.TranqualizingShot:CooldownRemains(BypassRecovery) <= 0 then
            for aH = 0, 40 do
                local aE, aE, aI, aJ, aE, aE, aE, aE, aE, aK = UnitBuff("target", aH)
                for aE, aL in pairs(K) do
                    if aL == aK then
    ...
    if F and O.WailingArrow:IsReady() and O.WailingArrow:CooldownRemains(BypassRecovery) <= 0 then
        if m.Cast(O.WailingArrow, nil, nil, nil) then
            a8 = 392060;
            return "queue Wailing Arrow"
        end
    elseif (not O.WailingArrow:IsUsableP() or O.WailingArrow:CooldownRemains() > 0) and F then
        ...
        m.Print("Wailing Arrow Queue is now " ..
    ...
    if i:IsActive() and select(2, GetStablePetInfo(N.Commons.SummonPetSlot)) ~= select(1, UnitName("pet")) and
        N.Commons.SummonPetSlot ~= 0 and not g:IsMoving() and not g:PrevGCD(1, O.DismissPet) and
        not g:IsCasting(O.DismissPet) then
    ...
    if (i:IsDeadOrGhost() or not UnitExists("pet")) and O.RevivePet:IsCastable() and not g:IsMoving() and
        N.Commons.SummonPetSlot ~= 0 then
        if p(O.RevivePet, nil) then
            a8 = 982;
            return "Revive Pet"
        end
    end
    local aM, aN, aE = GetSpellCooldown(34477)
    if g:AffectingCombat() and N.Commons.AutoMisdirect and not C then
        if aM + aN - GetTime() <= 0 and O.Misdirection:IsAvailable() and O.Misdirection:CooldownRemains(BypassRecovery) <=
    ...
    if not i:IsDeadOrGhost() and UnitExists("pet") and O.MendPet:IsCastable() and i:HealthPercentage() <=
        N.Commons.MendPetHighHP and not g:PrevGCD(1, O.MendPet) then
        if p(O.MendPet) then
            a8 = 1982;
            return "Mend Pet High Priority"
        end
    end
    if not g:AffectingCombat() and (u or N.BeastMastery.DBMSupport and
        (O.BestialWrath:IsCastable() and I and m.BossMods:GetPullTimer() < Z and m.BossMods:GetPullTimer() > 0 or
            m.BossMods:GetPullTimer() < 0.5 and m.BossMods:GetPullTimer() > 0)) and L.TargetIsValid() then
        a7 = au()
        if a7 then
            return a7
        end
