...
local function se(i)
    ...
    local a = e.SteelTrap:IsAvailable() and (e.SteelTrap:CooldownRemains() > (a:Focus() / (p - u) * l))
        ...
        s = 1259489
        return true
    elseif ((h and w and d and r and (not e.SteelTrap:IsAvailable() or a)) and ((TargetInRangePet30y) or not PetRangeAbility) and (i:AffectingCombat() or i:IsDummy() or i:NPCID() == 153285 or i:NPCID() == 168326 or UnitExists("boss1")) and n.Survival.KillCommandTargetSwap == "AutoSwap" and i:GUID() ~= t:GUID() and not f) then
        s = 999
        return true
    elseif ((h and w and d and r and (not e.SteelTrap:IsAvailable() or a)) and ((TargetInRangePet30y) or not PetRangeAbility) and (i:AffectingCombat() or i:IsDummy() or i:NPCID() == 153285 or i:NPCID() == 168326 or UnitExists("boss1")) and i:GUID() == t:GUID()) then
        o = 259489
        return true
    elseif (h and w and d and r and (not e.SteelTrap:IsAvailable() or a)) and ((TargetInRangePet30y) or not PetRangeAbility) and (i:AffectingCombat() or i:IsDummy() or i:NPCID() == 153285 or i:NPCID() == 168326 or UnitExists("boss1")) then
        return true
    end
...
local function Ee()
    ...
    if (a:BuffUp(e.CoordinatedAssault)) or not e.CoordinatedAssault:IsAvailable() then
        if (j == 1) then
    ...
    if q.Jotungeirr:IsEquippedAndReady() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
...
local function _e()
    if (a:BuffUp(e.CoordinatedAssault) or not e.CoordinatedAssault:IsAvailable()) and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
        ...
        if e.AncestralCall:IsCastable() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
        ...
        if e.Fireblood:IsCastable() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
    ...
    if e.LightsJudgment:IsCastable() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
    ...
    if e.BagofTricks:IsCastable() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) and (e.KillCommand:FullRechargeTime() > a:GCD()) then
    ...
    if e.Berserking:IsCastable() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) and (a:BuffUp(e.CoordinatedAssault) or not e.CoordinatedAssault:IsAvailable() or t:TimeToDie() < 13) then
...
local function Te()
    ...
    if true then
        ...
        if e.CoordinatedAssault:IsCastable() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
...
local function L()
    ...
    if e.MongooseBite:IsReady() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) and (a:BuffUp(e.MongooseFuryBuff) or a:Focus() + a:FocusCastRegen(e.KillCommand:ExecuteTime()) > a:FocusMax() or a:BuffUp(e.CoordinatedAssault) or not e.CoordinatedAssault:IsAvailable()) then
        if i(e.MongooseBite, nil, nil, not t:IsSpellInRange(e.MongooseBite)) then
            o = 259387
    ...
    if e.SerpentSting:IsReady() and (a:BuffUp(e.VipersVenomBuff)) then
        ...
        if g(t) and t:DebuffDown(e.SerpentSting) then
            if i(e.SerpentSting, nil, nil, not t:IsInRange(40)) then
                o = 259491
                return "serpent_sting bop 34"
            end
...
local function H()
    ...
    if e.CoordinatedAssault:IsReady() and k() and (t:IsInMeleeRange(8) or (EagleUp and t:IsInMeleeRange(40))) then
        if i(e.CoordinatedAssault, nil) then
            o = 266779
...
local function w()
    ...
    if a:HealthPercentage() < n.Commons.HealthstoneHP and q.Healthstone:IsReady() and q.Healthstone:CooldownRemains() <= 0 then
        if d.Cast(q.Healthstone, nil) then
            o = 40
    ...
    if a:HealthPercentage() < n.Commons.HealPotHP and q.CosmicHealPot:IsReady() and q.CosmicHealPot:CooldownRemains() <= 0 then
        if d.Cast(q.CosmicHealPot, nil) then
            o = 45
    ...
    if c.TargetIsValid() and (a:AffectingCombat() or R) and (t:AffectingCombat() or t:IsDummy() or t:NPCID() == 153285 or t:NPCID() == 168326 or t:NPCID() == 176581 or t:NPCID() == 176920 or t:NPCID() == 177892 or t:NPCID() == 182778 or t:NPCID() == 185402 or t:NPCID() == 183945 or t:NPCID() == 182074 or t:NPCID() == 184737 or t:NPCID() == 179733 or t:NPCID() == 115402 or t:NPCID() == 115406 or t:NPCID() == 115395 or UnitExists("boss1") or t:NPCID() == 115388 or R) and not a:IsDeadOrGhost() then
        ...
        if e.ExplosiveShot:IsCastable() and e.ExplosiveShot:IsAvailable() and ve and (t:IsInMeleeRange(10)) then
            if i(e.ExplosiveShot, nil, nil, not t:IsInRange(40)) then
                o = 212431
        ...
        if e.ArcaneTorrent:IsCastable() and (t:IsInMeleeRange(8)) and k() then
