...
elseif l.GUISettings.General.OpenerReset > 0 and L ~= nil and GetTime() > L and HeroRotationCharDB.Toggles[6] then HeroRotationCharDB
    ...
    UnitIsDead("target") ~= true then for ap = 0, 10 do local aM, aM, aN, aO, aP, aQ, aM, aM, aM, aR = UnitBuff("target"
    , ap) for aM, aS in pairs(G) do if aS == aR and aQ - aP + 500 / 1000 <= GetTime() then if l.Cast(R.Shiv, nil) then a8 = 5938; return "Shiv Enrage" end end end end end end local aT = g
