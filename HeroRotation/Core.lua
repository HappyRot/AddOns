...
local r = LibStub("LibDataBroker-1.1"):NewDataObject("HeroRotation", {
    ...
    OnTooltipShow = function(u)
        u:AddLine("HeroRotation", 1, 1, 1)
    end
})
function b.CoreInit()
    HeroRotationDB.db = LibStub("AceDB-3.0"):New("HeroRotationDB", {
        profile = {
            minimap = {
                hide = false
            }
...
function b.Print(...)
    if not q.General.SilentMode then
...
function b.GetTexture(x)
    ...
    if y then
        ...
        if not z[y] then
            if y >= 999900 then
                z[y] = "Interface\\Addons\\HeroRotation\\Textures\\" .. tostring(y)
            elseif x.TextureSpellID then
                z[y] = GetSpellTexture(x.TextureSpellID)
            else
                z[y] = GetSpellTexture(y)
            end
        end
        return z[y]
    end
...
local function H(x, I, J)
    ...
    if I then
        ...
        if J then
            ...
            N = g.TimeToXResourceMap[x:CostInfo(nil, "type")](x:Cost())
        end
        if N and N > 0 then
            if N > K + L - M then
                local O = M - F + N;
                if G == 0 and O > 0.25 or G > O or O - G > 0.5 then
                    G = O
                end
                K = F;
                L = G
...
function b.Cast(x, P, Q, R, J)
    ...
    if P or Q == "Cooldown" then
        if b.CastOffGCDOffset == 1 or b.CastOffGCDOffset == 2 and b.SmallIconFrame:GetIcon(1) ~= S then
            b.SmallIconFrame:ChangeIcon(b.CastOffGCDOffset, S, T, R)
            b.CastOffGCDOffset = b.CastOffGCDOffset + 1;
            x.LastDisplayTime = GetTime()
            return false
        end
    elseif Q == "Suggested" then
        b.CastSuggested(x, R)
    elseif Q == "SuggestedRight" then
        b.CastRightSuggested(x, R)
    else
        local U = 999910;
...
function b.CastAnnotated(x, P, X)
    local Y = b.Cast(x, P)
    if not P then
        b.MainIconFrame:OverlayText(X)
    end
    return Y
end
function b.CastPooling(x, J, R)
    return b.Cast(x, false, "Pooling", R, J)
end
local Z, _, a0, a1;
b.MaxQueuedCasts = 3;
local function a2(...)
...
function b.QueuedCast()
    for a3 = 1, 10 do
        if HeroRotationCharDB.Toggles[900 + a3] and not p[a3] then
            HeroRotationCharDB.Toggles[900 + a3] = not HeroRotationCharDB.Toggles[900 + a3]
        end
        if not HeroRotationCharDB.Toggles[900 + a3] and p[a3] then
            HeroRotationCharDB.Toggles[900 + a3] = not HeroRotationCharDB.Toggles[900 + a3]
        end
    end
    if #p >= 1 then
        for a3 = 1, #p do
            if p[a3] and b.Commons.Everyone.TargetIsValid() then
                local a9 = i(p[a3])
                if a9:IsUsableP() and a9:CooldownRemains(BypassRecovery) <= 0 and not g:PrevGCD(1, a9) and
                    not g:IsCasting(a9) then
                    if b.Cast(a9, nil, nil, nil) then
                        return a9:ID()
                    end
                elseif not a9:IsUsableP() or a9:CooldownRemains() > 0 or g:PrevGCDP(1, a9) then
                    HeroRotationCharDB.Toggles[900 + a3] = not HeroRotationCharDB.Toggles[900 + a3]
                    b.Print("QueueR for " .. a9:Name() .. " is now " ..
...
function b.CastLeft(x)
    if b.CastLeftOffset == 1 then
        b.CastLeftCommon(x)
    end
    return false
end
function b.CastLeftNameplate(ab, x)
    if b.CastLeftOffset == 1 and b.Nameplate.AddIcon(ab, x) then
        b.CastLeftCommon(x)
    end
    return false
end
...
function b.CmdHandler(ac)
    ...
    elseif ad == "smallcds" then
        ...
        b.Print("Small Cooldowns are now " ..
                    (HeroRotationCharDB.Toggles[5] and "|cff00ff00enabled|r." or "|cffff0000disabled|r."))
    elseif ad == "opener" then
        HeroRotationCharDB.Toggles[6] = not HeroRotationCharDB.Toggles[6]
        b.ToggleIconFrame:UpdateButtonText(6)
        b.Print("Opener is now " ..
    ...
    elseif ad == "scale" then
        if ae and af then
            af = tonumber(af)
            if af and type(af) == "number" and af > 0 and af <= 10 then
                if ae == "ui" then
                    b.MainFrame:ResizeUI(af)
                elseif ae == "buttons" then
    ...
    elseif ad == "resetbuttons" then
        b.ToggleIconFrame:ResetAnchor()
    elseif ad == "help" then
    ...
    elseif ad == "stopautoswap" then
        ...
        b.Print("Auto Swap is now " ..
                    (HeroRotationCharDB.Toggles[12] and "|cffff0000disabled|r." or "|cff00ff00enabled|r."))
    elseif ad == "amz" then
    ...
    elseif ad == "deathgrip" then
        ...
        b.Print("Death Grip Queue is now " ..
                    (HeroRotationCharDB.Toggles[14] and "|cff00ff00on|r." or "|cffff0000off|r."))
    elseif ad == "holdpot" then
        HeroRotationCharDB.Toggles[15] = not HeroRotationCharDB.Toggles[15]
        b.Print("DPS Pots are now " .. (HeroRotationCharDB.Toggles[15] and "|cffff0000disabled|r." or
                    "|cff00ff00enabled|r. Requires Show:Potions checked in settings"))
    elseif ad == "asphyxiate" then
        HeroRotationCharDB.Toggles[16] = not HeroRotationCharDB.Toggles[16]
        b.Print("Asphyxiate Queue is now " ..
                    (HeroRotationCharDB.Toggles[16] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "holdvanish" then
        ...
        b.Print("Vanish is now " .. (HeroRotationCharDB.Toggles[21] and "|cffff0000disabled|r." or
                    "|cff00ff00enabled|r. Requires Use Vanish checked in settings"))
    elseif ad == "cheapshot" then
        HeroRotationCharDB.Toggles[22] = not HeroRotationCharDB.Toggles[22]
        b.Print("Cheap Shot Queue is now " ..
    ...
    elseif ad == "kidney" then
        ...
        b.Print("Kidney Shot Queue is now " ..
    ...
    elseif ad == "flagellation" then
        ...
        b.Print("Flagellation Queue is now " ..
                    (HeroRotationCharDB.Toggles[28] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "legsweep" then
        ...
        b.Print("Leg Sweep Queue is now " ..
                    (HeroRotationCharDB.Toggles[34] and "|cff00ff00on|r." or "|cffff0000off|r."))
    elseif ad == "paralysis" then
        HeroRotationCharDB.Toggles[35] = not HeroRotationCharDB.Toggles[35]
        b.Print("Paralysis Queue is now " ..
                    (HeroRotationCharDB.Toggles[35] and "|cff00ff00on|r." or "|cffff0000off|r."))
    elseif ad == "ringofpeace" then
        HeroRotationCharDB.Toggles[36] = not HeroRotationCharDB.Toggles[36]
        b.Print("Ring of Peace Queue is now " ..
    ...
    elseif ad == "holdfelblade" then
        ...
        b.Print("Fel Blade is now " ..
                    (HeroRotationCharDB.Toggles[43] and "|cffff0000disabled|r." or "|cff00ff00enabled|r."))
    ...
    elseif ad == "feleruption" then
        ...
        b.Print("Fel Eruption Queue is now " ..
    ...
    elseif ad == "chaosnova" then
        ...
        b.Print("Chaos Nova Queue is now " ..
    ...
    elseif ad == "holdeyebeam" then
        ...
        b.Print("Eye Beam is now " ..
                    (HeroRotationCharDB.Toggles[47] and "|cffff0000disabled|r." or "|cff00ff00enabled|r."))
    ...
    elseif ad == "bladerushmelee" then
        ...
        b.Print("Blade Rush Melee Only is now " ..
                    (HeroRotationCharDB.Toggles[51] and "|cff00ff00enabled|r." or "|cffff0000disabled|r."))
    ...
    elseif ad == "forceaoe" then
        ...
        b.Print("ForceAoE is now " ..
                    (HeroRotationCharDB.Toggles[60] and "|cff00ff00enabled|r." or "|cffff0000disabled|r."))
    elseif ad == "massdispel" then
        HeroRotationCharDB.Toggles[61] = not HeroRotationCharDB.Toggles[61]
        b.Print("Mass Dispel Queue is now " ..
                    (HeroRotationCharDB.Toggles[61] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "startweave" then
        ...
        b.Print("Weaving is now " ..
                    (HeroRotationCharDB.Toggles[70] and "|cff00ff00enabled|r." or "|cffff0000disabled|r."))
    ...
    elseif ad == "autotaunt" then
        ...
        b.Print("AutoTaunt is now " ..
                    (HeroRotationCharDB.Toggles[72] and "|cff00ff00enabled|r." or "|cffff0000disabled|r."))
    ...
    elseif ad == "mightybash" then
        ...
        b.Print("Mighty Bash Queue is now " ..
                    (HeroRotationCharDB.Toggles[77] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "bloodboil" then
        ...
        b.Print("Blood Boil Queue is now " ..
                    (HeroRotationCharDB.Toggles[84] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "bindingshot" then
        ...
        b.Print("Binding Shot Queue is now " ..
    ...
    elseif ad == "intimidation" then
        ...
        b.Print("Intimidation Queue is now " ..
    ...
    elseif ad == "holdmisdirection" then
        ...
        b.Print("Misdirection is now " .. (HeroRotationCharDB.Toggles[125] and "|cffff0000disabled|r." or
                    "|cff00ff00enabled|r. (Requires AutoMisdirect checked in settings)"))
    ...
    elseif ad == "shadowfury" then
        ...
        b.Print("Shadowfury Queue is now " ..
                    (HeroRotationCharDB.Toggles[150] and "|cff00ff00on|r." or "|cffff0000off|r."))
    elseif ad == "healooc" then
        HeroRotationCharDB.Toggles[160] = not HeroRotationCharDB.Toggles[160]
        b.Print("Healing from OOC is now " ..
                    (HeroRotationCharDB.Toggles[160] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "vespertotem" then
        ...
        b.Print("Vesper Totem Queue has " ..
                    (HeroRotationCharDB.Toggles[162] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "purifyspirit" then
        ...
        b.Print("Purify Spirit Queue has " ..
                    (HeroRotationCharDB.Toggles[164] and "|cff00ff00on|r." or "|cffff0000off|r."))
    ...
    elseif ad == "chaosnova" then
        ...
        b.Print("Chaos Nova Queue is now " ..
                    (HeroRotationCharDB.Toggles[169] and "|cffff0000off|r." or "|cff00ff00on|r."))
    elseif tonumber(ad) then
        RunQueueLoop(tonumber(ad))
    else
...
do
    ...
    function b.AoEON()
        return HeroRotationCharDB.Toggles[2] and not ag[h:NPCID()]
    end
end
function b.ON()
